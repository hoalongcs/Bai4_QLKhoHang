    ...
    public partial class frmNhaCungCap : Form
    {
        ...
        public void Init()
        {
            if (dataGridView1.RowCount > 1)
            {
                try
                {
                    txtMaNCC.Text = dataGridView1.Rows[0].Cells[1].Value.ToString();
                    txtTenNCC.Text = dataGridView1.Rows[0].Cells[2].Value.ToString();
                    txtDiaChi.Text = dataGridView1.Rows[0].Cells[3].Value.ToString();
            ...
            try
            {
                if (MessageBox.Show("Bạn có chắc chắn muốn xóa dữ liệu không ???", "Question", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    ...
                    else if (dataGridView1.SelectedRows.Count > 1)
                    {
                        ...
                        {
                            ncc.DeleteNCC(dataGridView1.Rows[dataGridView1.SelectedRows[i].Index].Cells[1].Value.ToString());
                        }
                    }
                    HienThi();
        ...
        private void dataGridView1_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                txtMaNCC.Text = dataGridView1.Rows[e.RowIndex].Cells[1].Value.ToString();
                txtTenNCC.Text = dataGridView1.Rows[e.RowIndex].Cells[2].Value.ToString();
